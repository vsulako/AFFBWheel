...
- 6 осей: руль (X), газ (Y), тормоз (Z), сцепление (Rx), и еще две дополнительно (Ry, Rz) (например для стика).
- 32 кнопки. 
...
- инкрементальный энкодер или TLE5010+магнит
...
Изменяемые настройки руля можно менять с помощью команд по последовательному порту (Serial monitor в Arduino)  
...
- `fvaout`  
вкл/выкл отладочный вывод значений усилия обратной связи, скорости и ускорения руля в оси Rx, Ry, Rz.  
Это значительно быстрее чем вывод в последовательный порт.
...
- закомментируйте `#define STEER_TYPE ST_ENCODER`
...
В остальном подключения не отличаются от ProMicro, изменений не требуется.

### Альтернативные варианты подключения педалей:

#### Вариант 1: подтяжки.

Количество проводов для подключения педалей снижается до 4.
...
Это можно компенсировать, если добавить по паре дополнительных резисторов ([схема](images/pedals_ADS1015_resistors.png)) на каждый потенциометр (сопротивление подбирать, начать с 1/10 от сопротивления потенциометра).  
...
При одновременном использовании MCP23017 и ADS1015 они подключаются параллельно к одним и тем же пинам.

## Увеличение скорости опроса (опционально).

В файле `[arduino dir]\hardware\arduino\avr\cores\arduino\USBCore.cpp` найдите функцию `USB_Send`.
Замените кусок
```cpp
...
```
Удаление этой задержки позволит достичь более высокой частоты опроса (более 1000 циклов в секунду).  
Не знаю, приведет ли это к каким-либо глюкам,  у меня пока что все работает нормально.